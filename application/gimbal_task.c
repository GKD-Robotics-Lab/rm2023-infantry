...
/**
 * @brief          "gimbal_control" valiable initialization, include pid initialization, remote control data point
 * initialization, gimbal motors data point initialization, and gyro sensor angle point initialization.
 * @param[out]     init: "gimbal_control" valiable point
 * @retval         none
 */
/**
 * @brief          初始化"gimbal_control"变量，包括pid初始化，
 * 遥控器指针初始化，云台电机指针初始化，陀螺仪角度指针初始化
 ...
 * @retval         none
 */
static void calc_gimbal_cali(
    const gimbal_step_cali_t *gimbal_cali,
    uint16_t *yaw_offset,
...
void gimbal_task(void const *pvParameters)
{
    ...
    while (1)
    {
        ...
        shoot_can_set_current =
            shoot_control_loop(&gimbal_control);  // 射击任务控制循环 shoot_can_set_current: 拨弹电机电流
        ...
        //(int8_t)(gimbal_control.fric1_motor.speed/80), (int8_t)(gimbal_control.fric1_motor.speed_set/80),
        //																							(int8_t)(gimbal_control.fric2_motor.speed/80),
        //(int8_t)(gimbal_control.fric2_motor.speed_set/80));
    ...
    fp32 *min_pitch)
{
    ...
    {
        calc_gimbal_cali(
            &gimbal_control.gimbal_cali, yaw_offset, pitch_offset, max_yaw, min_yaw, max_pitch, min_pitch);
...
static void gimbal_init(gimbal_control_t *init)
{
    ...
    // 初始化yaw电机pid
    gimbal_PID_init(
        &init->gimbal_yaw_motor.gimbal_motor_absolute_angle_pid,
    ...
    gimbal_PID_init(
        &init->gimbal_yaw_motor.gimbal_motor_relative_angle_pid,
    ...
    PID_init(
        &init->gimbal_yaw_motor.gimbal_motor_gyro_pid,
        PID_POSITION,
        Yaw_speed_pid,
    ...
    // 初始化pitch电机pid
    gimbal_PID_init(
        &init->gimbal_pitch_motor.gimbal_motor_absolute_angle_pid,
    ...
    gimbal_PID_init(
        &init->gimbal_pitch_motor.gimbal_motor_relative_angle_pid,
    ...
    PID_init(
        &init->gimbal_pitch_motor.gimbal_motor_gyro_pid,
        PID_POSITION,
        Pitch_speed_pid,
...
static void gimbal_mode_change_control_transit(gimbal_control_t *gimbal_mode_change)
{
        ...
        gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_RAW)
    {
        ...
        gimbal_mode_change->gimbal_yaw_motor.last_gimbal_motor_mode != GIMBAL_MOTOR_GYRO &&
        gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO)
    {
        gimbal_mode_change->gimbal_yaw_motor.absolute_angle_set =
            gimbal_mode_change->gimbal_yaw_motor.absolute_angle;
    }
    else if (
        gimbal_mode_change->gimbal_yaw_motor.last_gimbal_motor_mode != GIMBAL_MOTOR_ENCONDE &&
        gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)
    {
        gimbal_mode_change->gimbal_yaw_motor.relative_angle_set =
            gimbal_mode_change->gimbal_yaw_motor.relative_angle;
    }
        ...
        gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_RAW)
    {
        ...
        gimbal_mode_change->gimbal_pitch_motor.last_gimbal_motor_mode != GIMBAL_MOTOR_GYRO &&
        gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO)
    {
        ...
        gimbal_mode_change->gimbal_pitch_motor.last_gimbal_motor_mode != GIMBAL_MOTOR_ENCONDE &&
...
static void gimbal_motor_absolute_angle_control(gimbal_motor_t *gimbal_motor)
{
    ...
    gimbal_motor->motor_gyro_set = gimbal_PID_calc(
        &gimbal_motor->gimbal_motor_absolute_angle_pid,
