...
/**
 * @brief          "chassis_move" valiable initialization, include pid initialization, remote control data point
 * initialization, 3508 chassis motors data point initialization, gimbal motor data point initialization, and gyro
 * sensor angle point initialization.
 * @param[out]     chassis_move_init: "chassis_move" valiable point
 * @retval         none
 */
/**
 * @brief          初始化"chassis_move"变量，包括pid初始化，
 * 遥控器指针初始化，3508底盘电机指针初始化，云台电机初始化，陀螺仪角度指针初始化
...
/**
 * @brief          when chassis mode change, some param should be changed, suan as chassis yaw_set should be now
 * chassis yaw
...
/**
 * @brief          set chassis control set-point, three movement control value is set by
...
static void chassis_init(chassis_move_t *chassis_move_init)
{
    ...
    // chassis angle PID
    // 底盘角度pid值
    const static fp32 chassis_yaw_pid[3] = { CHASSIS_FOLLOW_GIMBAL_PID_KP,
...
void chassis_mode_change_control_transit(chassis_move_t *chassis_move_transit)
{
    ...
    // 切入跟随底盘角度模式
        ...
        (chassis_move_transit->last_chassis_mode != CHASSIS_VECTOR_FOLLOW_CHASSIS_YAW) &&
    ...
    // 切入不跟随云台模式
        ...
        (chassis_move_transit->last_chassis_mode != CHASSIS_VECTOR_NO_FOLLOW_YAW) &&
...
/**
 * @brief          accroding to the channel value of remote control, calculate chassis vertical and horizontal speed
 * set-point
 *
...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    rc_deadband_limit(
        chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_X_CHANNEL], vx_channel, CHASSIS_RC_DEADLINE);
    rc_deadband_limit(
        chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_Y_CHANNEL], vy_channel, CHASSIS_RC_DEADLINE);
    ...
    {
        if (chassis_move_rc_to_vector->key_vx_ramp.out < 0)
    ...
    else if (xTaskGetTickCount() - last_key_tick < 800)  // 如果没有按下按键，则清空斜波输出
                                                         // (用于键盘和遥控器热切换)
    {
        //				ramp_to_zerro(&chassis_move_rc_to_vector->key_vx_ramp, RAMP_KEY_ADD_VX * 7);  //
        //逐渐减小到0，但斜率更大
        chassis_move_rc_to_vector->key_vx_ramp.out = 0;
        vx_set_channel = chassis_move_rc_to_vector->key_vx_ramp.out;
    }
    ...
    {
        if (chassis_move_rc_to_vector->key_vy_ramp.out < 0)
    ...
    else if (chassis_move_rc_to_vector->chassis_RC->key.v & CHASSIS_RIGHT_KEY)
    {
        ...
        last_key_tick = xTaskGetTickCount();
                                                         ...
                                                         // (用于键盘和遥控器热切换)
    {
        //				ramp_to_zero(&chassis_move_rc_to_vector->key_vy_ramp, RAMP_KEY_ADD_VY * 7);  //
        //逐渐减小到0，但斜率更大
        chassis_move_rc_to_vector->key_vy_ramp.out = 0;
        vy_set_channel = chassis_move_rc_to_vector->key_vy_ramp.out;
    }
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
    ...
    if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW)
    {
        ...
        chassis_move_control->vx_set = fp32_constrain(
            chassis_move_control->vx_set, chassis_move_control->vx_min_speed, chassis_move_control->vx_max_speed);
        chassis_move_control->vy_set = fp32_constrain(
            chassis_move_control->vy_set, chassis_move_control->vy_min_speed, chassis_move_control->vy_max_speed);
    }
    ...
    else if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_NO_FOLLOW_YAW)
    {
        ...
        else  // 如果未在小陀螺模式 (angle_set来自遥控器, chassis_no_follow_angle_pid用于以较慢的速度回正底盘)
        {
            ...
            // 计算旋转PID角速度
            chassis_move_control->wz_set = angle_set - PID_calc(
...
chassis_vector_to_mecanum_wheel_speed(const fp32 vx_set, const fp32 vy_set, const fp32 wz_set, fp32 wheel_speed[4])
{
    ...
    {
        // because the gimbal is in front of chassis, when chassis rotates, wheel 0 and wheel 1 should be slower and
        // wheel 2 and wheel 3 should be faster 旋转的时候， 由于云台靠前，所以是前面两轮 0 ，1 旋转的速度变慢，
        // 后面两轮 2,3 旋转的速度变快
...
static void chassis_control_loop(chassis_move_t *chassis_move_control_loop)
{
    ...
    chassis_vector_to_mecanum_wheel_speed(
        chassis_move_control_loop->vx_set,
