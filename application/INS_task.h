...
#define TEMPERATURE_PID_DEAD_BAND      0.0f

#define IMU_TEMP_PWM_MAX               5000 // mpu6500控制温度的设置TIM的重载值，即给PWM最大为 MPU6500_TEMP_PWM_MAX - 1

#define INS_TASK_INIT_TIME             7 // 任务开始初期 delay 一段时间

#define INS_ANGLE_YAW_ADDRESS_OFFSET   2
#define INS_ANGLE_PITCH_ADDRESS_OFFSET 1
#define INS_ANGLE_ROLL_ADDRESS_OFFSET  0
...
#define GYRO_OFFSET_MAX 0.005 // 校准后零漂的最大值，超过该值视为校准错误

extern void INS_task(void const *pvParameters);
extern void INS_cali_gyro_hook(fp32 cali_scale[3], fp32 cali_offset[3], uint16_t *time_count);
extern void INS_set_cali_gyro_hook(fp32 cali_scale[3], fp32 cali_offset[3]);
